    ...
    mouseController:setOnMove(
            function(entity, x, y, dx, dy)
                ...
                if isCamera then
                    ...
                    rotation.y = rotation.y - dy * 0.1
                end
                transform:setPosition(position)
                TPSPlayer(scene,MainCamera.id,model,MainCamera,0.26,0.4)
            end
            , scene
        )
end
...
function CreateCamera(scene, width, height)
    ...
    trans:setPosition(glm.vec3(0, -0.04, 0.4))
...
function TPSPlayer(scene,camera_id,model,camera,height,radius)
    ...
    local dot = glm.dot(up, target_dir)


    dot = glm.clamp(dot, -1.0, 1.0)

    local pitch_rad = math.acos(dot)
    local pitch_deg = glm.degrees(pitch_rad) - 90.0 


    pitch_deg = glm.clamp(pitch_deg, -45, 45)
